/**
 * Terminal color themes — ported 1:1 from t3code
 * `apps/mobile/src/features/terminal/terminalTheme.ts`.
 *
 * t3code resolves non-default theme ids from its Uniwind palette; this app
 * ships a single dark theme, so the Pierre palettes below are authoritative
 * and `getMobileTerminalTheme` keeps the same signature (themeId ignored).
 */
...
export function getMobileTerminalTheme(
  ...
  return getPierreTerminalTheme(scheme);
}
