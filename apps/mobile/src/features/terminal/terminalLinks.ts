/**
 * Terminal link detection — ported from t3code `apps/web/src/terminal-links.ts`.
 *
 * URL + file-path regexes, trailing-punctuation / unbalanced-delimiter
 * trimming, `~/cwd` + `:line:col` resolution. Buffer/wrapped-line helpers are
 * kept API-compatible but operate on plain-text line getters (our grid wraps
 * eagerly and carries no `isWrapped` metadata — every segment is one line).
 *
 * Mobile note: there is no Cmd/Ctrl-click here; taps open links directly.
 * `isTerminalLinkActivation` is kept for parity (web/desktop callers).
 */
...
export function extractTerminalLinks(line: string): TerminalLinkMatch[] {
  ...
  // t3code uses `.toSorted` (ES2023); this app targets an older lib, so sort
  // a fresh array — same non-mutating semantics, same order.
  return [...urlMatches, ...pathMatches].sort(
    (a: TerminalLinkMatch, b: TerminalLinkMatch) => a.start - b.start,
  );
}
...
export function isTerminalLinkActivation(
  event: Pick<{ metaKey: boolean; ctrlKey: boolean }, "metaKey" | "ctrlKey">,
