...
export type TerminalAttachStreamEvent =
  | ({ readonly type: "snapshot" } & TerminalEventBase & { readonly snapshot: TerminalSessionSnapshot })
  | ({ readonly type: "output" } & TerminalEventBase & { readonly data: string })
  | ({ readonly type: "exited" } & TerminalEventBase & {
      readonly exitCode: number | null;
      readonly exitSignal: number | null;
    })
  | ({ readonly type: "closed" } & TerminalEventBase)
  | ({ readonly type: "error" } & TerminalEventBase & { readonly message: string })
  | ({ readonly type: "cleared" } & TerminalEventBase)
  | ({ readonly type: "restarted" } & TerminalEventBase & { readonly snapshot: TerminalSessionSnapshot })
