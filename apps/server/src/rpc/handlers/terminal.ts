...
export function registerTerminalHandlers(reg: RpcRegistry, tm: TerminalManager): void {
  reg.registerZod(RpcMethod.terminalOpen, TerminalOpenInput, async (p) => tm.open(p as TerminalOpenInput));
  ...
  reg.registerZod(RpcMethod.terminalRestart, TerminalRestartInput, async (p) => tm.restart(p as TerminalRestartInput));
